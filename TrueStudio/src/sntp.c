...
//
//void SNTP_init(uint8_t s, uint8_t *ntp_server, uint8_t tz, uint8_t *buf,uint8_t addr_len)
//{
//	NTP_SOCKET = s;
//if(addr_len == 4){
...
//	NTPformat.dstaddr[2] = ntp_server[2];
//	NTPformat.dstaddr[3] = ntp_server[3];
//	NTPformat.dstaddr[4] = ntp_server[4];
//	NTPformat.dstaddr[5] = ntp_server[5];
//	NTPformat.dstaddr[6] = ntp_server[6];
//	NTPformat.dstaddr[7] = ntp_server[7];
//	NTPformat.dstaddr[8] = ntp_server[8];
//	NTPformat.dstaddr[9] = ntp_server[9];
//	NTPformat.dstaddr[10] = ntp_server[10];
//	NTPformat.dstaddr[11] = ntp_server[11];
//	NTPformat.dstaddr[12] = ntp_server[12];
//	NTPformat.dstaddr[13] = ntp_server[13];
//	NTPformat.dstaddr[14] = ntp_server[14];
//	NTPformat.dstaddr[15] = ntp_server[15];
//}
//	time_zone = tz;
//
//	data_buf = buf;
...
void SNTP_init( uint8_t *ntp_server, uint8_t tz, uint8_t *buf,uint8_t addr_len)
{
...
if(addr_len == 4){
	...
	NTPformat6.dstaddr[0] = ntp_server[0];
	NTPformat6.dstaddr[1] = ntp_server[1];
	NTPformat6.dstaddr[2] = ntp_server[2];
	NTPformat6.dstaddr[3] = ntp_server[3];
	NTPformat6.dstaddr[4] = ntp_server[4];
	NTPformat6.dstaddr[5] = ntp_server[5];
	NTPformat6.dstaddr[6] = ntp_server[6];
	NTPformat6.dstaddr[7] = ntp_server[7];
	NTPformat6.dstaddr[8] = ntp_server[8];
	NTPformat6.dstaddr[9] = ntp_server[9];
	NTPformat6.dstaddr[10] = ntp_server[10];
	NTPformat6.dstaddr[11] = ntp_server[11];
	NTPformat6.dstaddr[12] = ntp_server[12];
	NTPformat6.dstaddr[13] = ntp_server[13];
	NTPformat6.dstaddr[14] = ntp_server[14];
	NTPformat6.dstaddr[15] = ntp_server[15];
	time_zone = tz;

	data_buf = buf;

	uint8_t Flag;
	NTPformat6.leap = 0;           /* leap indicator */
	NTPformat6.version = 4;        /* version number */
	NTPformat6.mode = 3;           /* mode */
	NTPformat6.stratum = 0;        /* stratum */
	NTPformat6.poll = 0;           /* poll interval */
	NTPformat6.precision = 0;      /* precision */
	NTPformat6.rootdelay = 0;      /* root delay */
	NTPformat6.rootdisp = 0;       /* root dispersion */
	NTPformat6.refid = 0;          /* reference ID */
	NTPformat6.reftime = 0;        /* reference time */
	NTPformat6.org = 0;            /* origin timestamp */
	NTPformat6.rec = 0;            /* receive timestamp */
	NTPformat6.xmt = 1;            /* transmit timestamp */

	Flag = (NTPformat6.leap<<6)+(NTPformat6.version<<3)+NTPformat6.mode; //one byte Flag
	memcpy(ntpmessage,(void const*)(&Flag),1);
}
}
int8_t SNTP_run(uint8_t s, datetime *time,uint8_t addr_len,uint8_t ver)
{
	uint16_t RSR_len;
	uint32_t destip = 0;
	uint16_t destport;
	static uint16_t startindex = 0x20;//53; //last 8-byte of data_buf[size is 48 byte] is xmt, so the startindex should be 40
	NTP_SOCKET = s;
	...
	case SOCK_UDP:
		...
		if(ntp_retry_cnt<0xFFFF)
		{
			...
			{
				if(addr_len == 16){
					sendto(NTP_SOCKET,ntpmessage,sizeof(ntpmessage),NTPformat6.dstaddr,ntp_port,addr_len);
					ntp_retry_cnt++;
				}
				else if(addr_len ==4){
	...
	case SOCK_CLOSED:
     	switch(ver)
           {
           case AS_IPV4:
               socket(NTP_SOCKET,Sn_MR_UDP4, ntp_port, 0x00);
               break;
           case AS_IPV6:
               socket(NTP_SOCKET,Sn_MR_UDP6, ntp_port,0x00);
               break;
           case AS_IPDUAL:
               socket(NTP_SOCKET,Sn_MR_UDPD, ntp_port, 0x00);
               break;
            }
		break;
	}
